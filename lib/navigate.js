...
import { refreshExplorer, resetSearch, goUpDirectory } from './navigate/explorer.js';
...
async function mainLoaded() {
  ...
  document.getElementById('go-up').addEventListener('click', goUpDirectory);
  document.getElementById('go-beginning').addEventListener('click', resetSearch);
  document.getElementById('add-secret').addEventListener('click', onAddSecretClick);
  document.getElementById('vault-search').addEventListener('keyup', onSearchKeyup);
}
...
async function onSearchKeyup(event) {
  ...
  await storage.set(StorageKeys.INPUT_VALUE, searchInput.value);
