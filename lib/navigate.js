...
/* global browser storePathComponents */
...
async function mainLoaded() {
  ...
  document.getElementById('add-secret').addEventListener('click', onAddSecretClick, false);
}

async function onAddSecretClick() {
  try {
    const data = await openCreateSecretDialog();
    await putSecret(data);
  } catch (e) {
    if (!isCancelled(e)) notify.error(e.message);
  }
}

async function onEditSecretClick(fullPathName) {
  try {
    const data = await openEditSecretDialog(getItemName(fullPathName));
    await putSecret(data, true);
  } catch (e) {
    if (!isCancelled(e)) notify.error(e.message);
  }
}
...
function displaySecret(fullPathName) {
  ...
  editButton.addEventListener('click', function () {
    onEditSecretClick(fullPathName);
  });
...
/*
  ...
  Invokes `callback` with the value of the selected key.
 */
function chooseSecretKey(secretContent, callback) {
  pickSecretKey(secretContent)
    .then(callback)
    .catch((e) => {
      if (!isCancelled(e)) notify.error(e.message);
    });
