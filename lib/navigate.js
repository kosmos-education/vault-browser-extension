    ...
    if (autoSearch){
      ...
      if (autoSearch){
        try{
          ...
          if (found){
            notify.info("Secrets trouvés pour "+mainDomain, {
            removeOption: true,
            });
          }else{
            await refreshExplorer()
            notify.info("Aucun secret trouvé pour "+mainDomain, {
              removeOption: true,
            });
          } 
        }catch (e) {}
...
function displaySecret(fullPathName) {
  ...
  const primaryContent = document.createElement('button');
  primaryContent.addEventListener('click', function () {
    getSecretContent(fullPathName).then((content) => { fillCredentialsInBrowser(getItemName(fullPathName), content.value); }).catch(e => { console.log(e) })
  ...
  copyValueButton.addEventListener('click', function () {
    getSecretContent(fullPathName).then((content) => { copyStringToClipboard(content.value); })
...
//Returns secret content from the API when click
async function getSecretContent(urlPath) {
...
const searchHandler = function (e) {
  if (e.key === 'Enter') {
    ...
    const found = searchSecrets(searchInput.value);
    if (!found){
